...
// #define WALL ((y < 10) && (fabs(x + 10) < 0.5))
...
// event init (t = 0) {
//   refine (sq(x+10)  < sq(1) && y  < 11 && level < maxlevel);
...
//   vorticity (u, omega);
//   boundary ({omega});
//   // view (fov = 7, width = 500, height = 500);
...
// 	tx = 0, ty = 0, bg = {0.3,0.4,0.6}, width = 500,
...
event adapt (i++) 
  adapt_wavelet ({cs, b, u}, (double[]){0.01, 0.1, 0.05, 0.05}, maxlevel, 5);
