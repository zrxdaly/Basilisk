...
event init (t = 0) {
  ...
  // vertex scalar phih[];
  // double min_x = xw, max_x = xw + W, min_y = Y0, max_y = Y0 + H, min_z = 0, max_z = 0; //L0/2-Lw/2
  // foreach_vertex() {
  //   coord cc = {x, y};
...
event adapt (i++) 
  adapt_wavelet ((scalar *){fan, b, u}, (double[]){0.1, 0.1, 0.05, 0.05, 0.05}, maxlevel, 4);
